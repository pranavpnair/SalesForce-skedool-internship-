...
import com.sforce.soap.enterprise.sobject.EventRelation;
      ...
      if (login()) {
       ...
       //  describeGlobalSample();
        ...
        // querySample();
         ...
         event.put("title","Added Event from terminal");
         event.put("location","Visakhapatnam,Andhra Pradesh");
         event.put("date","2015-06-15");
         event.put("start","14:00:00");
         event.put("end","15:00:00");
         event.put("timezone","America/Los_Angeles");
         event.put("description","Added this event as sync from terminal for testing");
         String[] invitees = {"johnsmith@gmail.com","pranavpradeepnair@gmail.com","naveen@skedool.it"};
         addEvent(event,invitees);
         // Log out
         logout();
      }
   }

   private void addEvent(Map<String,String> eventData, String[] invitees) {
	   Event event = new Event();
	   event.setSubject(eventData.get("title"));
	   event.setLocation(eventData.get("location"));
	   String startDateTime = eventData.get("date") + "T" + eventData.get("start") + "Z";
	   String endDateTime = eventData.get("date") + "T" + eventData.get("end") + "Z";
	   //event.set;
	   
//	   //converting from specific timezone to utc timezone
//	   localTimeZone = pytz.timezone(eventData.get("timezone"));
//	   naiveStartDateTime = datetime.datetime.strptime(startDateTime,"%Y-%m-%dT%H:%M:%SZ")
//	   naiveEndDateTime = datetime.datetime.strptime(endDateTime,"%Y-%m-%dT%H:%M:%SZ")
//	
//	   localStartDateTime = localTimeZone.localize(naiveStartDateTime, is_dst=None)
//	   localEndDateTime = localTimeZone.localize(naiveEndDateTime, is_dst=None)
//	
//	   UtcStartDateTime = localStartDateTime.astimezone(pytz.utc)
//	   UtcEndDateTime = localEndDateTime.astimezone(pytz.utc)
//	
//	   EventStart = UtcStartDateTime.strftime("%Y-%m-%dT%H:%M:%S")
//	   EventEnd = UtcEndDateTime.strftime("%Y-%m-%dT%H:%M:%S")
	
//	   newEvent.put("StartDateTime",startDateTime);
//	   newEvent.put("EndDateTime",endDateTime);
	   event.setDescription(eventData.get("description"));
       String[] acceptedinvitees = new String[5];
       int l=0;
	   for(int k=0;k< invitees.length;k++){
		   String soqlQuery = "SELECT Id, Name FROM Contact WHERE Email = " + "'" + invitees[k] + "'";
		      try {
	 ...
	 event.setAcceptedEventInviteeIds(acceptedinvitees);
	 Event[] e1 = {event};
	 try {
		connection.update(e1);
...
// Constructor
   ...
   private boolean login() {
      ...
      String username = "pranavpradeepnair@gmail.com";
      String password = "d^T@dS85UrD7AbcsmfI9jBa7vlIRu81mP";
   ...
   private void querySample() {
      String soqlQuery = "SELECT FirstName, LastName, Email, Id FROM Contact";
      try {
         QueryResult qr = connection.query(soqlQuery);
         boolean done = false;
